...
def proc_word_reduction(word):
    if word == 'don\'t' or word == 'don`t':
        return 'do not'
    if word == 'didn\'t' or word == 'didn`t':
        return 'did not'
    if word == 'can\'t' or word == 'can`t':
        return 'can not'
    if word == 'won\'t' or word == 'won`t':
        return 'will not'
    if word == 'haven\'t' or word == 'haven`t':
        return 'have not'
    if word == 'hadn\'t' or word == 'hadn`t':
        return 'had not'
    if '\'re' in word:
        return word.split('\'')[0] + ' are'
    if '`re' in word:
        return word.split('`')[0] + ' are'
    if '\'m' in word:
        return word.split('\'')[0] + ' am'
    if '`m' in word:
        return word.split('`')[0] + ' am'
    if '\'s' in word:
...
def proc_reductions(questions, answers):
    questions_res = []
    answers_res = []
    for string in questions:
        question = ""
        for word in string.split(' '):
            seq = proc_word_reduction(word)
            question += " " + str(seq)
        questions_res.append(question)
    ...
    return questions_res, answers_res
...
def filter_unk(qtokenized, atokenized, w2idx):
    ...
    for qline, aline in zip(qtokenized, atokenized):
        ...
        #             pass
        if unk_count_q == 0 and unk_count_a == 0:
...
def process_data():
    ...
    # remove reductions from sentences
    questions, answers = proc_reductions(questions, answers)
